...
# Работаем из своей папки: если скрипт запустили из чужого каталога (например,
# из /root при проверке через sudo), команда очистки старых копий не сможет
# вернуться в него и напишет ошибку
cd "$DIR"
